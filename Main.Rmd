...
```
```{r}
xm.source_test_7_lag <- zoo(dt[[2]])[523:530]
xm.source_test_7_lag
```
...
desaison_test_7_lag <- desaison_test_7_lag 
...
#Hypothèse nulle : la série a une racine sur le cercle unité / H1: Stationarité
pp.test(desaison)
#On veut une p-valeur inférieure à 0.05 : c'est le cas avec la série initiale
```
